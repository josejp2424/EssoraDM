...
void App::Run()
{
	...
	if (!testing) {
		...
		/* EssoraDM boot splash hook.
		 * This runs before X starts, so a framebuffer splash can be shown
		 * and then closed cleanly before the greeter appears.
		 */
		system("/etc/essoradm/pre-start 2>/dev/null || true");
